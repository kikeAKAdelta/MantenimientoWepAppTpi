 ...
 */
@Stateless
@Path("sv.edu.uesocc.ingenieria.tpi2018.entity.diagnostico")
public class DiagnosticoFacadeREST extends AbstractFacade<Diagnostico> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Diagnostico entity) {
        super.edit(entity);
    }
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Diagnostico> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
