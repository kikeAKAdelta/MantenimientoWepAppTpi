 ...
 */
@Stateless
@Path("entity.excepcioncalendario")
public class ExcepcionCalendarioFacadeREST extends AbstractFacade<ExcepcionCalendario> {
    ...
    public void edit(@PathParam("id") Integer id) {
        if (id>0) {
            ExcepcionCalendario exp=new ExcepcionCalendario();
            exp=super.find(id);
            if (exp != null) {
                super.edit(exp);
            }
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<ExcepcionCalendario> findRange(@PathParam("from") @DefaultValue("1") Integer from, @PathParam("to") @DefaultValue("50") Integer to) {
