 ...
 */
@Stateless
@Path("sv.edu.uesocc.ingenieria.tpi2018.entity.tipomtto")
public class TipomttoFacadeREST extends AbstractFacade<Tipomtto> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Tipomtto entity) {
        super.edit(entity);
    }
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Tipomtto> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
