...
public class MttoDetalleFacadeREST extends AbstractFacade<MttoDetalle> {
    ...
    public void create(MttoDetalle entity) {
        if (entity != null) {
            super.create(entity);
        }
    ...
    public void edit(@PathParam("id") Integer id) {
        if (id>0) {
            MttoDetalle mt=new MttoDetalle();
            mt=super.find(id);
            if (mt != null) {
                super.edit(mt);
            }
    ...
    public MttoDetalle find(@PathParam("id") Integer id) {
        if (id>0) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<MttoDetalle> findRange(@PathParam("from") @DefaultValue("1") Integer from, @PathParam("to") @DefaultValue("50") Integer to) {
