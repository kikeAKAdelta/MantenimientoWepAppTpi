...
//        super.create(entity);  //Se crea una instancia administrada y persistente.
          if (entity != null) {
            super.create(entity);
        }
    }
    ...
    public void edit(@PathParam("id") Integer id) {
        //super.edit(entity); //Combina el estado de la entidad dada en el contexto de persistencia actual.
        if (id>0) {
            Calendario calendario=new Calendario();
            calendario=super.find(id);
            if (calendario != null) {
                super.edit(calendario);
            }
        }
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Calendario> findRange(@PathParam("from") @DefaultValue("1") Integer from, @DefaultValue("50")@PathParam("to") Integer to) {
